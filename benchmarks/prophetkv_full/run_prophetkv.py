...
CACHE=Path('/tmp/ucm-prophetkv-qwen3-ruler100-longbenchv2-64k-c4096-20260918')
INSTALLED=REPO/'.envs/cacheblend/lib/python3.10/site-packages/ucm'
SOURCES=tuple('benchmarks/prophetkv_full/'+n for n in ('run_prophetkv.py','cacheblend_prophetkv.py','prophetkv_common.py',
    'prophetkv_data.py','prophetkv_gpu.py','prophetkv_report.py','test_prophetkv.py','cacheblend_ruler.py','PROPHETKV.md'))+tuple(str(p.relative_to(REPO)) for p in sorted((REPO/'benchmarks/prophetkv_full/method/prophetkv').glob('*.py')))+('ucm/sparse/factory.py','ucm/sparse/blend/blend.py','ucm/sparse/blend/selection.py')
...
def prepare():
    with lock():
        ...
        shutil.copytree(REPO/'benchmarks/prophetkv_full/method/prophetkv',private/'sparse/prophetkv',ignore=shutil.ignore_patterns('__pycache__'))
        ...
        p=dict(study='ProphetKV UCM/vLLM port',schema_version=1,model=str(MODEL),model_revision=MODEL.name,
            ...
            cases=list(CASES),tasks=list(TASKS),context_targets=list(LENGTHS),chunk_sizes=list(CHUNKS),samples_per_task=100,
            ...
            measured_requests=len(samples)*len(CASES),samples=samples,dataset_hashes=datasets,
            sources={n:sha(REPO/n) for n in SOURCES},private_ucm_hashes=hashes(private),installed_ucm_hashes=hashes(INSTALLED),
            gpu_uuids=GPU_UUIDS,prompt_format='native non-thinking chat; numbered block-padded chunks; fresh suffix of max(256, complete query tail tokens)',
        ...
        dump(ROOT/'dataset_manifest.json',dict(seed=42,hashes=datasets,selection='RULER rows 0–99; all LongBench v2 contexts with pinned tokenizer count < 65536'))
        dump(ROOT/'progress.json',dict(state='prepared',validated=0,target=p['measured_requests']))
        return p
...
def verify():
    ...
    if len(p['samples'])*len(p['cases'])!=p['measured_requests']:raise ValueError('Scope mismatch')
    return p
...
def launch(meta,case,path,smoke=False):
    ...
    for attempt in range(2):
        ...
        cmd=[str(PYTHON),'-u',str(ROOT/'source/benchmarks/prophetkv_full/cacheblend_prophetkv.py'),'--protocol',str(ROOT/'protocol.json'),
...
def equivalence(directory):
    ...
    receipt=dict(complete=True,passed=all(r['passed'] for r in errors),control='100% vs no-cache',
        compared='all fresh suffix hidden states after every decoder layer',tolerance='relative RMS < 0.03 per layer',layers=errors)
...
def numerical():
    ...
    with (directory/'cpu.log').open('w') as log:
        subprocess.run([str(PYTHON),'-m','unittest','discover','-s',str(ROOT/'source/benchmarks/prophetkv_full'),'-p','test_prophetkv.py','-v'],
            cwd='/tmp',env=cpu_env(),stdout=log,stderr=subprocess.STDOUT,check=True)
    idle(1)
    try:
        with (directory/'gpu.log').open('w') as log:
            subprocess.run([str(PYTHON),str(ROOT/'source/benchmarks/prophetkv_full/test_prophetkv.py'),'--gpu'],cwd='/tmp',env=environment(1),
...
def smoke(p,meta):
    length,chunk=meta['context_target'],meta['chunk_size']
    directory=ROOT/f"smoke/{meta['id']}";gate=directory/'validation.json'
    if gate.exists():
        receipt=json.loads(gate.read_text())
        if (receipt.get('complete') and receipt.get('protocol_sha256')==sha(ROOT/'protocol.json')
...
def per_gpu(p,gpu,samples,pilot):
    for meta in samples:
        ...
        pending=[case for case in order(meta,pilot) if not valid(ROOT/'records'/meta['id']/f'{case}.json',meta,case)]
...
def run(pilot_only=False):
    with lock():
        ...
        try:
            ...
            ruler=[m for m in p['samples'] if m['dataset']=='ruler']
            lb=[m for m in p['samples'] if m['dataset']=='longbench_v2']
            ...
            for meta in audits.values():smoke(p,meta)
            for dataset in ('ruler','longbench_v2'):
                samples=[m for m in p['samples'] if m['dataset']==dataset]
                phase(p,samples,f'sweep-{dataset}-4096')
                report(all_records(p),'sweep')
            verify()
            rows=all_records(p)
            if len(rows)!=p['measured_requests']:raise RuntimeError('Incomplete final matrix')
            ...
            if state=='complete':dump(ROOT/'final/validation.json',dict(complete=True,validated=p['measured_requests'],
