...
CASES=('baseline','prophetkv-10','prophetkv-20','prophetkv-50','prophetkv-90','cacheblend-10','cacheblend-90')
...
def audit_capture(worker,enable,output_path=None):
    ...
    def hook(index):
        def capture(module,args,result):
            ...
            worker.prophet_model_audit[index]=total[-256:].detach().cpu()
        return capture
...
def main():
    ...
    if p['sources']['benchmarks/cacheblend_prophetkv.py']!=sha(__file__):raise RuntimeError('Worker changed')
    ...
    if args.case=='populate':
        ...
        if args.smoke and args.case in ('baseline','prophetkv-100'):
            llm.collective_rpc(audit_capture,kwargs={'enable':True})
        ...
        output=result.outputs[0];value,_=score(sample,output.text)
        dp=args.output.with_suffix('.diagnostics.json');dump(dp,diagnostics)
        record=dict(**provenance,schema_version=2,case=args.case,smoke=args.smoke,dataset='ruler',label=sample['label'],
            ...
            finish_reason=output.finish_reason,score=value,references=sample['source_metadata']['references'],
