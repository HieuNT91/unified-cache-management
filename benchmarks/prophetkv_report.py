...
def gate(rows,length):
    rows=[r for r in rows if r['context_target']==length and r['chunk_size']==512 and r['source_row']<10]
    expected=60*7
    if len(rows)!=expected:raise ValueError('Pilot lacks all 420 validated measurements')
    table=stats(rows)
    baseline=next(r for r in table if r['task']=='macro' and r['case']=='baseline')
    candidates=[r for r in table if r['task']=='macro' and r['case'] in ('prophetkv-10','prophetkv-20','prophetkv-50')]
    passed=[r['case'] for r in candidates if r['accuracy']>=baseline['accuracy']-.05 and r['paired_speedup']>1]
    receipt=dict(complete=True,passed=bool(passed),passing_rates=passed,context_target=length,chunk_size=512,
        independent_prompts=60,measurements=420,baseline=baseline,candidates=candidates,
        rule='macro accuracy >= baseline - 0.05 AND paired geometric TTFT speedup > 1.0')
    dump(ROOT/f'gates/pilot-{length}.json',receipt)
    return receipt
...
def report(rows,state,final=False):
    ...
    text=f'# ProphetKV UCM/vLLM port\n\nState: **{state}**. {len(rows)}/7,560 validated measurements.\n\n'
