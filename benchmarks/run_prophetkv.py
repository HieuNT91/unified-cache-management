...
CACHE=Path('/tmp/ucm-prophetkv-qwen3-ruler30-20260918')
INSTALLED=REPO/'.envs/cacheblend/lib/python3.10/site-packages/ucm'
SOURCES=tuple('benchmarks/'+n for n in ('run_prophetkv.py','cacheblend_prophetkv.py','prophetkv_common.py',
...
def prepare():
    with lock():
        ...
        p=dict(study='ProphetKV UCM/vLLM port',schema_version=1,model=str(MODEL),model_revision=MODEL.name,
            ...
            cases=list(CASES),tasks=list(TASKS),context_targets=list(LENGTHS),chunk_sizes=list(CHUNKS),samples_per_task=30,
            pilot_samples_per_task=10,measured_requests=7560,samples=samples,dataset_hashes=datasets,
        ...
        dump(ROOT/'dataset_manifest.json',dict(seed=42,hashes=datasets,selection='source row ordinals 0–29'))
        dump(ROOT/'progress.json',dict(state='prepared',validated=0,target=7560))
        return p
...
def verify():
    ...
    if len(p['samples'])*len(p['cases'])!=7560:raise ValueError('Scope mismatch')
    return p
...
def progress(p,state):
    with PROGRESS:
        ...
        dump(ROOT/'progress.json',dict(state=state,validated=n,target=7560,updated_at=time.time()))
        return n
...
def launch(meta,case,path,smoke=False):
    ...
    for attempt in range(2):
        ...
        cmd=[str(PYTHON),'-u',str(ROOT/'source/benchmarks/cacheblend_prophetkv.py'),'--protocol',str(ROOT/'protocol.json'),
...
def numerical():
    ...
    with (directory/'cpu.log').open('w') as log:
        subprocess.run([str(PYTHON),'-m','unittest','discover','-s',str(ROOT/'source/benchmarks'),'-p','test_prophetkv.py','-v'],
            cwd='/tmp',env=cpu_env(),stdout=log,stderr=subprocess.STDOUT,check=True)
    idle(1)
    try:
        with (directory/'gpu.log').open('w') as log:
            subprocess.run([str(PYTHON),str(ROOT/'source/benchmarks/test_prophetkv.py'),'--gpu'],cwd='/tmp',env=environment(1),
...
def smoke(p,length,chunk):
    ...
    meta=next(m for m in p['samples'] if m['context_target']==length and m['chunk_size']==chunk and m['label']=='cwe' and m['source_row']==0)
...
def order(meta,pilot):
    ...
    offset=meta['source_row']%len(cases)
...
def run(pilot_only=False):
    with lock():
        ...
        try:
            ...
            numerical()
            from prophetkv_report import gate,report
            for length in LENGTHS:
                smoke(p,length,512)
                samples=[m for m in p['samples'] if m['context_target']==length and m['chunk_size']==512 and m['source_row']<10]
                phase(p,samples,f'pilot-{length}-512',True)
                receipt=gate(all_records(p),length)
                if not receipt['passed']:
                    state='performance_gate_unmet';report(all_records(p),state);return
            if pilot_only:state='pilots_complete';return
            for length in LENGTHS:smoke(p,length,4096)
            phase(p,p['samples'],'full-sweep')
            verify()
            rows=all_records(p)
            if len(rows)!=7560:raise RuntimeError('Incomplete final matrix')
            ...
            if state=='complete':dump(ROOT/'final/validation.json',dict(complete=True,validated=7560,
