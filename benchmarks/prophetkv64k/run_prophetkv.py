...
CACHE=Path('/tmp/ucm-prophetkv-qwen3-ruler10-64k-chunks-gpu34-20260918')
INSTALLED=REPO/'.envs/cacheblend/lib/python3.10/site-packages/ucm'
SOURCES=tuple('benchmarks/prophetkv64k/'+n for n in ('run_prophetkv.py','cacheblend_prophetkv.py','prophetkv_common.py',
...
def prepare():
    with lock():
        ...
        p=dict(study='ProphetKV UCM/vLLM port',schema_version=1,model=str(MODEL),model_revision=MODEL.name,
            ...
            chunk_12000_adaptation="11999 tokens including numbered marker, then block padding to 12032; final chunk may be shorter",measured_requests=1000,samples=samples,dataset_hashes=datasets,
        ...
        dump(ROOT/'progress.json',dict(state='prepared',validated=0,target=1000))
        return p
...
def verify():
    ...
    if len(p['samples'])*len(p['cases'])!=1000:raise ValueError('Scope mismatch')
    return p
...
def progress(p,state):
    with PROGRESS:
        ...
        dump(ROOT/'progress.json',dict(state=state,validated=n,target=1000,updated_at=time.time()))
        return n
...
def launch(meta,case,path,smoke=False):
    ...
    for attempt in range(2):
        ...
        cmd=[str(PYTHON),'-u',str(ROOT/'source/benchmarks/prophetkv64k/cacheblend_prophetkv.py'),'--protocol',str(ROOT/'protocol.json'),
...
def numerical():
    ...
    with (directory/'cpu.log').open('w') as log:
        subprocess.run([str(PYTHON),'-m','unittest','discover','-s',str(ROOT/'source/benchmarks/prophetkv64k'),'-p','test_prophetkv.py','-v'],
            cwd='/tmp',env=cpu_env(),stdout=log,stderr=subprocess.STDOUT,check=True)
    idle(3)
    try:
        with (directory/'gpu.log').open('w') as log:
            subprocess.run([str(PYTHON),str(ROOT/'source/benchmarks/prophetkv64k/test_prophetkv.py'),'--gpu'],cwd='/tmp',env=environment(3),
    ...
    with lock():
        ...
        try:
            ...
            for batch in (CHUNKS[:2],CHUNKS[2:]):
                with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
                    pending={pool.submit(smoke,p,65536,chunk) for chunk in batch}
                    while pending:
                        done,pending=concurrent.futures.wait(pending,timeout=1,return_when=concurrent.futures.FIRST_COMPLETED)
                        for future in done:
                            if future.exception():STOP.set()
            ...
            for chunk in CHUNKS:
                ...
                phase(p,samples,f'sweep-65536-{chunk}')
                report(all_records(p),'sweep')
            verify()
            rows=all_records(p)
            if len(rows)!=1000:raise RuntimeError('Incomplete final matrix')
            ...
            if state=='complete':dump(ROOT/'final/validation.json',dict(complete=True,validated=1000,
